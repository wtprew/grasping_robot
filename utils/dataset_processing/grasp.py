...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1, handle_as_ggcnn=False):
    """
    ...
    for grasp_point_array in local_max:
        ...
        # print('Output bin:', grasp_point[0])
        g = Grasp(grasp_point[-2:], grasp_angle)
        if width_img is not None:
            g.length = abs(width_img[grasp_point])
            g.width = g.length/2
