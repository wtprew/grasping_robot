...
def transform(image, pixels):
	'''
	Transform image from opencv image to 
	...
	y_centre = y/2
	radius = pixels/2
	cropped = image[x_centre-radius: x_centre+radius, y_centre-radius:y_centre+radius, :]
	# cv2.imshow('cropped', cropped)
	rgb = cv2.cvtColor(cropped, cv2.COLOR_BGR2RGB)
	# cv2.imshow('rgb', rgb)
	# cv2.waitKey(0)
	transpose = np.transpose(rgb, (2,0,1)).astype(np.float32)
	tensor = torch.from_numpy(transpose).unsqueeze(0)
	return tensor
