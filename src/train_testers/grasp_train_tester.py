...
class GraspTrainTester(BaseTrainTester):
    ...
    def test(self):
        ...
        self.logger.info(f"Testing {self._net_name} on {self._dataset}. Saving results to {self._results_path}.")
        self.net.eval()
        self.net.mode = 'test'
        if self._use_cuda:
            self.net.cuda()
        ...
        self.results_files = os.path.join(self._results_path, self._net_name)

        # Forward pass on test set
        results = {}
        for batch in tqdm(self.data_loader, disable=self._run_on_server):
            ...
            for cnt, (grasp_list, _id) in enumerate(zip(grasps, ids)):
                ...
                results[_id], j_output = self.evaluator.step(
                    q_img, ang_img, width_img, grasp_list
                )
                if self._jacquard_output:
                    # if cnt%10==0: #Return every 10th grasp to not overload server
                    with open(self.results_files + '_jacquard_output.txt', 'a') as f:
                        f.write(os.path.split(_id)[1] + '\n')
                        f.write(j_output + '\n')
        self.evaluator.print_stats(self._net_name)
        with open(self.results_files +'.json', 'w') as fid:
    ...
    def _loss_function(self, batch, pos, cos, sin, width, graspness, bins,
        ...
        # Compute loss
        if self._use_smooth_l1_loss:
            pos_loss = F.smooth_l1_loss(pos, pos_imgs)
            if self._use_positional_loss:
                cos_loss = F.smooth_l1_loss(pos_imgs*cos, pos_imgs*cos_imgs)
                sin_loss = F.smooth_l1_loss(pos_imgs*sin, pos_imgs*sin_imgs)
                width_loss = F.smooth_l1_loss(pos_imgs*width, pos_imgs*width_imgs)
                positional_loss = pos_loss + cos_loss + sin_loss + width_loss
            else:
                cos_loss = F.smooth_l1_loss(cos, cos_imgs)
                sin_loss = F.smooth_l1_loss(sin, sin_imgs)
                width_loss = F.smooth_l1_loss(width, width_imgs)
        else:
            pos_loss = F.mse_loss(pos, pos_imgs)
            if self._use_positional_loss:
                cos_loss = F.mse_loss(pos_imgs*cos, pos_imgs*cos_imgs)
                sin_loss = F.mse_loss(pos_imgs*sin, pos_imgs*sin_imgs)
                width_loss = F.mse_loss(pos_imgs*width, pos_imgs*width_imgs)
                positional_loss = pos_loss + cos_loss + sin_loss + width_loss
            else:
        ...
        if self._use_positional_loss:
            ret_losses['positional'] = positional_loss
        return (
            self._num_of_bins * (
