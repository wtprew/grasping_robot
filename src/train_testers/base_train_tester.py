...
class BaseTrainTester:
    ...
    def _set_from_config(self, config):
        ...
        self._loss_path = os.path.join(config.paths['loss_path'], config.net_name)
        self._models_path = os.path.join(config.paths['models_path'], config.net_name)
        ...
        self._results_path = os.path.join(config.paths['results_path'], config.net_name)
        self._use_smooth_l1_loss = config.use_smooth_l1_loss
        self._use_angle_loss = config.use_angle_loss
        self._use_binary_map = config.use_binary_map
        self._use_positional_loss = config.use_positional_loss
        ...
        self._use_dropout = config.use_dropout
        ...
        self._jacquard_output = config.jacquard_output
        self._run_on_server = config.run_on_server
    ...
    def train(self, optimizer, scheduler=None, epochs=1):
        ...
        model_path_name = os.path.join(self._models_path, self._net_name + '.pt')
    ...
    def _train_epoch(self, epoch, model_path_name, loss_history):
        ...
        self.writer.add_scalar('train_loss/loss', accum_loss, epoch)
        self.writer.add_scalar('val_loss', val_loss, epoch)
        self.writer.add_scalar('learning_rate', curr_lr, epoch)
        ...
        for n, l in print_loss.items():
            self.writer.add_scalar('val_loss/' + n, l, epoch)
        ...
        with open(self._loss_path + '.json', 'w') as fid:
    ...
    def test(self):
        ...
        self.logger.info(f"Testing {self._net_name}")
    ...
    def _check_for_checkpoint(self, model_path_name):
        ...
        if any(epochs_to_resume):
            ...
            self.net.train()
            if os.path.exists(self._loss_path + '.json'):
                with open(self._loss_path + '.json') as fid:
