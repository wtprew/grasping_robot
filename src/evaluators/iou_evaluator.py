...
class IoUEvaluator:
    ...
    def __init__(self, config):
        ...
        self._handle_as_discrete_bins = config.handle_as_discrete_bins
    ...
    def step(self, q_img, angle_img, width_img, targets):
        ...
        max_iou, j_output = calculate_iou_match(
            q_img, angle_img, width_img,
            targets, self._handle_as_ggcnn, self._handle_as_discrete_bins, self._num_of_bins, self._im_size, self._jaw_size)
        self._detected[:int(np.floor(max_iou)) + 1] += 1
        self._gt += 1
        return max_iou, j_output
    ...
    def print_stats(self, name=''):
        ...
        print("Accuracy@0.40: %f" % acc_per_thres[40])
        ...
        plt.savefig(join(self._results_path, name, name + '_accuracy.png'))
        plt.close()


def calculate_iou_match(q_img, angle_img, width_img, ground_truth_bbs,
                        handle_as_ggcnn, handle_as_discrete_bins, num_of_bins, im_size, jaw_size):
    """
    ...
    if handle_as_discrete_bins:
        center, angle, length, width = [
            grasp_point[-2:], (np.pi/num_of_bins)*grasp_point[0], width_img[grasp_point],
    ...
    scale = 1024/im_size
    j_output = f'{center[1]*scale:.2f}; {center[0]*scale:.2f}; {-1*angle*180/np.pi:.2f}; {length*scale:.2f}; {width*scale:.2f}'

    # Return max IoU
    return max(100 * iou(det, grasp) for grasp in ground_truth_bbs), j_output
