...
class Image:
	...
	def sal_normalise(self):
		"""
		Normalise the image by converting to float [0,1] and zero-centering
		"""
		self.img = self.img.astype(np.float32)/255.0
		self.img -= self.img.mean()
		self.img = cv2.normalize(self.img, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX)
...
class DepthImage(Image):
	...
	def normalise(self):
		...
		# self.img = (self.img - 1.45)/(1.5741-0.7932)
		# self.img = cv2.normalize(self.img, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX)
