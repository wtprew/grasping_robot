...
def visualise_output(x, y, net, grasp_success=0, title=None):
	...
	q_out, cos_out, sin_out, width_out, depth_out = net(x.unsqueeze(0))
	
	# if x.shape[0] == 4:
		# image = x.detach().cpu().numpy()[1:,:,:].transpose((1,2,0))
	# elif x.shape[0] == 3:
	...
	f_axes[0][0].imshow(image)
	f_axes[0][0].set_title('input')
	f_axes[0][0].axis('off')
	f_axes[0][1].imshow(y[-1].detach().cpu().squeeze().numpy())
	f_axes[0][1].set_title('Perfect Depth')
	f_axes[0][1].axis('off')
	...
	gs = detect_grasps(q_out, ang_out, width_img=w_out)
	for g in gs:
		g.plot(f_axes[1][0])
	...
	depth = topil(depth_out.detach().squeeze().cpu())
	f_axes[1][1].imshow(depth)
	f_axes[1][1].set_title('Depth output')
	f_axes[1][1].axis('off')

	f_axes[2][0].imshow(q_out, cmap='jet', vmin=0, vmax=1)
	f_axes[2][0].set_title('Q')
	f_axes[2][0].axis('off')

	f_axes[2][1].imshow(ang_out, cmap='hsv', vmin=-np.pi / 2, vmax=np.pi / 2)
	f_axes[2][1].set_title('Angle')
	f_axes[2][1].axis('off')
...
class GraspRectangles:
	...
	@classmethod
	def load_first_grasp_from_jacquard_file(cls, fname, scale=1.0):
		"""
		Load grasp rectangles from a Jacquard dataset file.
		:param fname: Path to file.
		:param scale: Scale to apply (e.g. if resizing images)
	...
	@classmethod
	def extreme_grasps_from_file(cls, fname):
		"""
		Load grasp rectangles from a Jacquard dataset file.
		:param fname: Path to file.
		:param scale: Scale to apply (e.g. if resizing images)
		:return: list of grasps in order: min x, max x, min y, max y
		"""
		grs = []
		with open(fname) as f:
			for l in f:
				x, y, theta, w, h = [float(v) for v in l[:-1].split(';')]
				# index based on row, column (y,x), and the Jacquard dataset's angles are flipped around an axis.
				grs.append(Grasp(np.array([y, x]), -theta/180.0*np.pi, w, h))
		
		x_sort = sorted(grs, key=lambda x: x.x)
		y_sort = sorted(grs, key=lambda x: x.y)

		grasp_rs = []

		grasp_rs.append(x_sort[0].as_gr)
		grasp_rs.append(x_sort[-1].as_gr)
		grasp_rs.append(y_sort[0].as_gr)
		grasp_rs.append(y_sort[-1].as_gr)
	...
	def draw(self, shape, binary_map=False, gaussian=2, bins=1, filter=False, compact=True):
		"""
		...
		for gr in self.grs:
			if compact:
				rows, cols = gr.compact_polygon_coords((shape[0], shape[1]))
			else:
				rows, cols = gr.polygon_coords(shape)
			
			b_map = np.zeros((shape[0], shape[1]))
			b_map[rows, cols] = 1
			angle = np.arctan2(
				-gr.points[1, 0] + gr.points[0, 0],
				gr.points[1, 1] - gr.points[0, 1]
			)
			angle = (angle + np.pi / 2) % np.pi
			ang_bin = int(np.floor(bins * angle / np.pi))
			angle -= np.pi / 2
			width = np.sqrt(
				(gr.points[1, 1] - gr.points[0, 1]) ** 2
				+ (gr.points[1, 0] - gr.points[0, 0]) ** 2
			)
		...
		return pos_out, ang_out, width_out, grasp_out

	def draw_discrete(self, shape, binary_map=False, gaussian=2, bins=1, filter=False):
		"""
		Plot all GraspRectangles as solid rectangles in a numpy array, e.g. as network training data.
		:param shape: output shape
		:param position: If True, Q output will be produced
		...
		for gr in self.grs:
			...
			width_out[ang_bin][(b_map > 0) & (width_out[ang_bin] == 0)] = width
			width_out[ang_bin] = np.maximum(b_map * width, width_out[ang_bin])
		return pos_out, width_out, grasp_out
