...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def _set_init(self):
        ...
        self._handle_as_discrete_bins = self._config.handle_as_discrete_bins
        ...
        self._use_filter = self._config.use_filter
        ...
        self._gaussian_drop_off = self._config.gaussian
        self._compact = self._config.compact
        if self._config.use_rgbd_img:
            self.features.add('images')
        self._suffix = '_'+str(self._im_size)
    ...
    def get_grasp_targets(self, anno):
        ...
        elif self._handle_as_discrete_bins:
            draw_func = self.draw_discrete
        else:
            draw_func = self.draw
    ...
    def draw_as_ggcnn(grasps, shape):
        ...
        for grasp in grasps:
            rows, cols = compact_polygon_coords(grasp.astype(np.int), shape, compact=True)
            angle = np.arctan2(
                -grasp[1, 0] + grasp[0, 0],
                grasp[1, 1] - grasp[0, 1]
            )
    ...
    def draw_discrete(self, grasps, shape):
        ...
        for grasp in grasps:
            ...
            rows, cols = discrete_compact_polygon_coords(grasp, bins, shape)
            ...
            angle -= np.pi / 2
            angle = ((np.pi / bins) * ang_bin) - ((np.pi/bins)*0.5)
            width = np.sqrt(
                (grasp[1, 1] - grasp[0, 1]) ** 2
                + (grasp[1, 0] - grasp[0, 0]) ** 2
            )
            ...
            if not self._use_binary_map:
                ...
                gauss_grid = np.exp(-(grid_x ** 2 + grid_y ** 2) / self._gaussian_drop_off)
                if self._use_filter:    
                    gauss_grid[(gauss_grid > 0) & (gauss_grid < self._filter)] = self._filter
    ...
    def draw(self, grasps, shape):
        ...
        for grasp in grasps:
            # Compute polygon and values to fill
            rows, cols = compact_polygon_coords(grasp, shape, compact=self._compact)
            if not rows.tolist() or not cols.tolist():
                continue
            b_map = np.zeros(shape)  # auxiliary binary map
            b_map[rows, cols] = 1
...
def compact_polygon_coords(grasp, shape=None, compact=True):
...
def discrete_compact_polygon_coords(grasp, bins=1, shape=None):
    """Return pixels within the centre third of the grasp rectangle."""
    center = grasp.mean(axis=0)  # .astype(np.int)
    angle = np.arctan2(-grasp[1, 0] + grasp[0, 0], grasp[1, 1] - grasp[0, 1])
    angle = (angle + np.pi / 2) % np.pi - np.pi / 2
